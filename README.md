...
| `project-bootstrap` | **Greenfield project creation gate** (`/apex:new`) — empty directory → scaffolded, git-initialized, CI'd, apex-wired project: intake paragraph → `architecture-design` (the decisions) → **official-generator scaffold** (never hand-rolled — hand-rolled skeletons fossilize) → `docs/` convention + CLAUDE.md skill-gate wiring → least-privilege CI baseline (audited by `cicd-review`) → first **walking-skeleton** PRD queued via `/apex:prd` (one feature, no backlog seeding). **ADOPT mode** for existing repos: wires the convention in, touches nothing else. Adversarial counter: speculative-structure hunt + the day-two test (could a second contributor run tests + find the next task in 15 min?). YAGNI guard: throwaway scripts scaffold and stop. |
| `release-readiness` | **The SHIP gate** (`/apex:release`) — 5-pass release event for the USER's project (not apex's own releases): **semver decision audited against the actual diff** since the last tag (a "patch" with a changed default fails) / changelog + notes written for users (what's new for me / what breaks / what must I do) / **readiness gate that blocks the tag** (suite green at the exact SHA, version bumped in every manifest, migration + config notes, **rollback path stated while sober**) / mechanics (annotated tag → **build from the tag, never the working tree** → publish → verify; never delete-recreate a pushed tag) / post-release **bake watch** (signals from `observability-review`'s contract, abort criteria pre-named, bad bake → `incident-retro`). Fires for versioned release events; continuous deploys are `deployment-review`'s territory. |
| `cicd-review` | **Pipeline-as-code gate** — treats a CI workflow as a **privileged program that runs other people's input**. 5 passes: trigger + privilege surface (least-privilege `permissions:`, the `pull_request_target` trap, untrusted-interpolation injection) / supply chain (third-party actions pinned by **full SHA**, no curl-pipe-bash, cache poisoning) / structure (each job maps to a named `test-strategy` CI tier, `timeout-minutes` everywhere, concurrency groups) / secrets + identity (**OIDC federation over stored cloud keys**, environment protection rules for deploys) / determinism + debuggability (lockfile installs, actionable failure logs, idempotent re-runs). Adversarial counter: *you are a malicious PR author* — walk each job as the attacker. Auto-nudged by `suggest-skill-on-edit` on workflow-file paths. |
| `deployment-review` | **Deploy + infra gate** — fires when a diff alters HOW software reaches an environment (deploy workflows, IaC, env promotion). 5 passes: deploy identity (workload identity / OIDC per cloud — Azure federated credential, AWS role-assume, GCP WIF; blast-radius question per role) / environment promotion (**build once, promote the same artifact**; config from the environment) / rollout shape (health-gated cutover, canary vs all-at-once **justified** per change, sequencing against expand→migrate→contract) / **rollback decided before deploy** (previous artifact deployable now, time-to-rollback bounded, not-rollback-safe steps flagged with forward-fix notes) / IaC review (**the plan is the review artifact**, remote locked state, destroy/replace lines called out — a replace on a database routes to `data-migration-review`). Instantiates `architecture-design` Pass 4's deploy model per-change. |
| `ui-design-review` | **The LOOK-AT-IT gate for user-facing UI** — catches the failure mode no other gate sees: visually wrong code that passes every test because nobody *looked*. 5 passes: design-system conformance (first-affordance applied to UI — name the rejected existing component before inventing one) / **the five states designed up front** (loading / empty / error / partial / ideal — "whatever the framework does" fails) / interaction + **WCAG 2.2 AA as Definition-of-Done** (keyboard-walk the flow, visible focus, ≥24×24 targets, no drag-only) / the **screenshot loop** (render → screenshot via agent-driven browser → critique vs the design → iterate, ≤3 rounds then human; screenshots attach to the PR) / visual-regression promotion of stabilized key screens to `test-strategy` Layer 7. Extends `verification-before-completion` to pixels; enforces `rules/frontend.md` rather than recalling it. |
| `council-review` | **Three-seat review council for the four highest-stakes freezes only** (architecture freeze · auth/payment/tenant/crypto design freeze · irreversible data migration · public API freeze) — extends `adversarial-pair` with the seat the pair lacks: completeness steelman + security/failure adversary + **operability/simplicity skeptic**, context-isolated, **one round, no debate**. Reconciliation by rule: two seats independently agree → blocker; explicit disagreement → **to the human verbatim, never averaged**. Includes the **model-routing table** (deep-reasoning class for the adversary seat and the reconciliation; a fast model never holds a council seat or decides a freeze). Explicitly NOT a persona swarm (the survey-rejected BMAD pattern): seats review one frozen-candidate artifact, never author. |
...
| `/apex:new` | `project-bootstrap` | Create a new project from scratch (or ADOPT apex in an existing one) — intake → `architecture-design` → official-generator scaffold → `docs/` + CLAUDE.md + CI-baseline wiring → first walking-skeleton PRD. |
| `/apex:release` | `release-readiness` | Cut a release of THIS project — semver audited against the diff, user-facing changelog, readiness gate (tests at the SHA, migration notes, rollback path stated), tag → build-from-tag → publish, post-release bake watch. |
...
When done, remind me to start a NEW Claude Code session (slash commands only register at session start) and type `/apex:help` — I should see the 17-command cheat sheet (`/apex:flow`, `/apex:prd`, `/apex:design`, `/apex:impl-plan`, `/apex:adversarial-pair`, …). Optionally, run `/apex:setup` to install the recommended companion plugins.
```
...
> **Note:** The CLAUDE.md stubs above use short names like `apex-flow` and `python-review` (the model's routing name). Only the 17 **entry-point** commands have an interactive `/apex:` slash form (e.g. `/apex:flow`); the review gates below are **skills** the model fires automatically — they are not in the slash menu.
