...
**Pass condition:** Each PR in the stack names its test scope. Integration tests are explicitly mapped to numbered PRD scenarios. Each PRD scenario from `apex:prd-review` Pass 2 has an owner PR in the stack.

**Adversarial counter-pass:** Name a PRD scenario that no PR in this stack covers. If found, either a layer needs a test added or the stack is missing a piece of feature surface. Also: name a test that no PRD scenario justifies — that's gold-plating or an undocumented requirement.
